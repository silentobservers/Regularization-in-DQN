        ...
        def build_layers(s, c_names, n_l1, w_initializer, b_initializer, trainable):
            if trainable:
                ...
                with tf.variable_scope('l3_eva'):
                    w3 = tf.get_variable('w3', [n_l1, self.n_actions], initializer=w_initializer, collections=c_names,  trainable=trainable)
                ...
                with tf.variable_scope('l1_tar'):
                    w4 = tf.get_variable('w4', [self.n_features, n_l1], initializer=w_initializer, collections=c_names,
                                         trainable=trainable)
                    b4 = tf.get_variable('b4', [1, n_l1], initializer=b_initializer, collections=c_names,
                                         trainable=trainable)
                    l4 = tf.nn.relu(tf.matmul(s, w4) + b4)

                with tf.variable_scope('l2_tar'):
                    w5 = tf.get_variable('w5', [n_l1, n_l1], initializer=w_initializer, collections=c_names,
                                         trainable=trainable)
                    b5 = tf.get_variable('b5', [1, n_l1], initializer=b_initializer, collections=c_names,
                                         trainable=trainable)
                    l5 = tf.nn.relu(tf.matmul(l4, w5) + b5)

                with tf.variable_scope('l3_tar'):
...
################tf.contrib.layers.xavier_initializer()tf.random_normal_initializer(0., 0.3)
        ...
        with tf.variable_scope('loss'):
            if self.prioritized:
                ...
                tf.get_variable('w1''w2''w3', shape=(), regularizer=tf.contrib.layers.l2_regularizer(0.00001))
                self.loss = tf.reduce_mean(tf.squared_difference(self.q_target, self.q_eval)) + tf.losses.get_regularization_loss()
