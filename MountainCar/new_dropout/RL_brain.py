...
class DQNPrioritizedReplay:
    ...
    def _build_net(self):
        ...
        self.q_eval_nodrop = tf.placeholder(tf.float32, [None, self.n_actions], name='Q_eval_nodrop')
...
################tf.contrib.layers.xavier_initializer()tf.random_normal_initializer(0., 0.3)
        ...
        with tf.variable_scope('loss'):
            if self.prioritized:
                ...
                self.loss = tf.reduce_mean(tf.squared_difference(self.q_target, self.q_eval)+0.01*tf.squared_difference(self.q_eval_nodrop, self.q_eval))
    ...
    def learn(self):
        ...
        if self.prioritized:
            ...
            _, self.cost = self.sess.run([self._train_op, self.loss],
                                         feed_dict={self.s: batch_memory[:, :self.n_features],
                                                    self.q_target: q_target,
                                                    self.q_eval_nodrop: q_next_nodrop,
                                                    keep_pro: 1})
